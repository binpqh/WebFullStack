...
export const fetchListCategory = createAsyncThunk("Category/fetchData", async () => {
  return await GetAllCategory();
});
export const updateCategory = createAsyncThunk("Category/updateData", async (category: ICategoryResult) => {
  return await UpdateCategory(category);
});
export const createCategory = createAsyncThunk("Category/createData", async (category: ICategoryResult) => {
  return await CreateCategory(category);
});
export const deleteCategory = createAsyncThunk("Category/deleteData", async (category: ICategoryResult) => {
...
const categorySlice = createSlice({
  ...
  extraReducers: (builder) => {
    //dc ma`
    builder
      .addCase(fetchListCategory.pending, (state: any, action) => {
      ...
      .addCase(deleteCategory.fulfilled, (state, action: any) => {
        ...
        const newlistCate = temp.filter((item: ICategoryResult) => item.categoryId !== action.payload.categoryId);
        return {
